...
Build each investigator's prompt from this template; fill in the placeholders. Append the single category playbook `skill://why/references/sources/<source>.md` matching this investigator's evidence category (see `skill://why/references/source-playbook.md` for the index). If the target code looks defensive (null checks, retry logic, timeout handling, rate limiting, feature flags, egress guards, OOM handlers), also append `skill://why/references/sources/incident-postmortem.md` for the incident-flavored queries to run inside its own source.
