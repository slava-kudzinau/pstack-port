...
5. Loop, one hypothesis per iteration:
   - Hand the change to a subagent using your configured hillclimb model role with a tight scope; supervise and review the diff rather than typing it (the `skill://principle-guard-the-context-window` principle skill). When several independent hypotheses are live, fan them to parallel subagents, each in its own worktree so they can't collide (the `skill://principle-separate-before-serializing-shared-state` principle skill).
   ...
   Each iteration ends in a check before the next begins (the `skill://principle-sequence-verifiable-units` principle skill). If the run is unattended, borrow only the wake mechanism from `skill://autonomous-run`, not its stop rule. This playbook's stop criteria below govern, so a plateau means pivot, not stop.
...
8. Run `skill://opening-a-pr` with the accepted commits stacked in the order they landed, so the metric's climb reads top to bottom.
