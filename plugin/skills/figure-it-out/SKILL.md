...
- The definition of done as a falsifiable predicate (the **prove-it-works** principle skill, `skill://principle-prove-it-works`). "Done well" has to be checkable.
...
Decompose into atomic, independently-landable units. Sequence riskiest-unknown-first so option value stays high. Scaffold and verification come before features (the **foundational-thinking** principle skill, `skill://principle-foundational-thinking`).

- Build the verification harness before the work, with the baseline captured from the pre-change state, so the check reads as "old value vs new value".
- For one-way-door design decisions, run the **architect** skill (`skill://architect`; it runs **arena**, `skill://arena`) with diverse, isolated, opinionated candidates and a read-only judge on a different model family. Skip it for mechanical work whose shape is already concrete. A second arena over a settled design is over-engineering (the **laziness-protocol** principle skill, `skill://principle-laziness-protocol`).
- Decide what fans out. Parallelize only across genuine seams, and give each worker its own worktree or branch (the **separate-before-serializing-shared-state** principle skill, `skill://principle-separate-before-serializing-shared-state`). Don't over-fan.
...
Check the whole against the Phase A predicate on the real product, not just the harness. Encode any recurring correction as a gate, a lint rule, a check, or a script, so the win can't silently regress (the **encode-lessons-in-structure** principle skill, `skill://principle-encode-lessons-in-structure`).
