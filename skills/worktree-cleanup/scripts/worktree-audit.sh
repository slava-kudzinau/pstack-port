...
# status: 'adapted'
# note: 'The transcript scan (upstream worktree-audit.sh:25-27,63-72) read
# the upstream vendor's per-project agent-transcripts text dir, which OMP has
# no equivalent of. OMP records prompt history in the SQLite database at
# ~/.omp/agent/history.db (table history: prompt, created_at, cwd,
# session_id), so LAST_CHAT is a max(created_at) query over rows whose cwd is
# the worktree or a path under it, or whose prompt text names it. Override the
# database path with PSTACK_HISTORY_DB.'
#
...
# Prompt-history database: OMP's history.db, see the provenance header above.
hist_db="${PSTACK_HISTORY_DB:-$HOME/.omp/agent/history.db}"
now=$(date +%s)
...
git worktree list --porcelain | awk '/^worktree /{print $2}' | while read -r wt; do
	...
	# Most recent prompt history that operated in this worktree: cwd at or
	# under the path, or prompt text naming it. The trailing "/" or quote in
	# the text match keeps glint-482 from matching glint-482-r37.
	last="-"; last_ts=0
	if [ -f "$hist_db" ] && command -v sqlite3 >/dev/null 2>&1; then
		q=$(printf '%s' "$wt" | sed "s/'/''/g")
		ts=$(sqlite3 "$hist_db" \
			"SELECT IFNULL(max(created_at),0) FROM history WHERE cwd='$q' OR cwd LIKE '$q/%' OR prompt LIKE '%/$q/%' OR prompt LIKE '%\"$q\"%';" 2>/dev/null)
		if [ -n "$ts" ] && [ "$ts" -gt 0 ] 2>/dev/null; then
			last_ts=$ts
			last=$(date -r "$ts" '+%Y-%m-%d' 2>/dev/null)
		fi
	fi
