...
metadata:
  upstream: 'pstack/skills/why/SKILL.md'
...
Launch all matching investigators in a single `task` batch call (`{context, tasks[]}`) so they run concurrently. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Don't ask one agent to cover multiple sources.
