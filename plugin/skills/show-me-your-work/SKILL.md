...
Write each entry the way you'd tell a teammate what you did. Plain words, concrete actions, no AI speak or abstract jargon (`skill://unslop` applies to log text too).

Use the helper `skill://show-me-your-work/scripts/log.sh <logfile> <phase> <decision> <why> <evidence> <result>` (`bash` auto-resolves the `skill://` URI to its file path). It stamps `ts`, writes the header on first use, strips stray tabs/newlines, and prefixes any cell starting with `=`, `+`, `-`, or `@` with a single quote. A bare `printf` appending a row works too, but mind those same bytes if cells come from generated or user-supplied text.
...
- Prefer evidence produced by committed scripts over hand-made one-offs (`skill://principle-encode-lessons-in-structure`).

## Audit the log against the transcript

At the end of the run, before handing back, check the log told the truth. Sessions persist as JSONL at `~/.omp/agent/sessions/<encoded-cwd>/<timestamp>_<sessionId>.jsonl`; read this run's session, or use `history://<id>` for a subagent's transcript. Don't glob broadly across `~/.omp/agent/sessions/`; that reads unrelated private sessions from other projects. Walk the log against what actually happened:
...
Other skills route their audit trail here instead of inventing one. Reference it as `skill://show-me-your-work` and let it own the format. Don't restate the columns.
