...
Transcripts live at `~/.claude/projects/<slug>/<uuid>.jsonl`, where `<slug>` is the workspace path with each `/` and each `_` turned into `-` (so `/Users/you/proj` becomes `-Users-you-proj`). Every line is one chat message.
