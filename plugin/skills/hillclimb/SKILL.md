...
5. Loop, one hypothesis per iteration:
   - Hand the change to a subagent using the caller's configured process/code model role (default `grok-4.6-fast-xhigh`) with a tight scope. Supervise and review the diff rather than typing it (the `skill://principle-guard-the-context-window` principle skill). When several independent hypotheses are live, fan them to parallel subagents, each in its own worktree (the `skill://principle-separate-before-serializing-shared-state` principle skill).
   ...
   Each iteration ends in a check before the next begins (the `skill://principle-sequence-verifiable-units` principle skill). If the run is unattended, borrow only the wake mechanism from `skill://autonomous-run`, not its stop rule.
6. Push past the first plateau. On a stall, several rejects in a row, pivot category, combine near-misses, re-read the source, or try something more radical before concluding the hill is climbed. Correctness and simplicity outrank the number. Revert a win that breaks behavior, and keep a simplification that holds the number (the `skill://principle-laziness-protocol` principle skill).
...
8. Run `skill://opening-a-pr` with the accepted commits stacked in the order they landed.
