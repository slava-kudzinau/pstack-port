...
Companion to the **how** skill (`skill://how`) and the **why** skill (`skill://why`). `how` tells you what the code does. `why` tells you why it's shaped that way. Blast radius tells you what it breaks somewhere else.
...
6. For a big or wide change, run it through the **arena** skill (`skill://arena`). Ask several models the same question and merge the answers. Different models catch different real bugs.
...
Write it through the **unslop** skill (`skill://unslop`), cite real code, and strip anything private before it goes anywhere public.
