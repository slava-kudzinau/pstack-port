...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `skills/verify-*/` at the project root). Several candidates → ask which one; none → stop and point at `skill://create-verification-skill` instead of inventing a target.
