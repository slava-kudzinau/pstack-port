...
3. Plan the fix from the trace. If it crosses a function boundary, `skill://architect` first. Delegate implementation to a subagent via the `task` tool using the caller's configured process/code model role (default `grok-4.6-fast-xhigh`). Review the diff. Capture a post-fix trace.
