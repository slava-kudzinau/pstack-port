...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's transcript via `history://<id>`, using the agent id each candidate's spawn returned. Do not read sessions outside the candidates you spawned this run. That crosses session boundaries and reads private transcripts from unrelated work. Look at which files each candidate actually opened. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
