...
Spawn the comment-sicko subagent. Act on accepted findings.
...
1. Spawn the `task` tool with `agent: "comment-sicko"`. Pass the scope. Do not restate its rules.
...
4. Implement the smallest root-cause fix in scope. Remove every named workaround. If the root cause is out of scope, land the smallest in-scope fix and report the rest open. The `skill://principle-fix-root-causes` and `skill://principle-redesign-from-first-principles` skills guide intent only. Neither authorizes widening the fence nor fixing instances outside it. Never bolt on symptom guards.
