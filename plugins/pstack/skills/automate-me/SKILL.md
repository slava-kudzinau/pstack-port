...
This skill orchestrates three others: an inline mining pass (see step 1), the plugin's own authoring playbook at `playbooks/authoring-a-skill.md`, and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Locate the active workspace's transcripts before fanning out. Claude Code keeps each project's transcripts under `~/.claude/projects/<slug>/`, newest first. Use only that path. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Author it with the plugin's own `playbooks/authoring-a-skill.md` playbook. Placement:
...
- `playbooks/authoring-a-skill.md`: skill authoring process and writing guidelines.
