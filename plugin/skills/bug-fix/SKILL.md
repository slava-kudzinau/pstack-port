...
3. Plan the fix. If it crosses a function boundary, `skill://architect` first. Delegate implementation to a subagent via the `task` tool using the caller's configured process/code model role (default `grok-4.6-fast-xhigh`) with a specific scope. Review the diff.
...
5. Stage the commits so the failing repro lands before the fix in git history. See `skill://tdd` for the failing-test-first cadence when the bug has a cheap local test path. Skip it when the test would be expensive, integration-heavy, or unclear.
   This is the canonical `skill://principle-sequence-verifiable-units` principle skill, the failing test first and the fix on top.
