...
| make-bot-ui | skill | rules | markdown skill | DROP | — | De-scoped from porting (user decision, 2026-08-31). Upstream drives Cursor's Routines/webhook automation (`update_state` routine tool, `sendNotification` card, `[routine]`/`<webhook_event>` wake) — no OMP equivalent exists; reference port agrees: it kept the page-hosting/secret-hygiene/Tailscale sections but dropped the routine machinery (CHANGES 0.9.11). `skills/make-bot-ui/` deleted |
...
| de-slop | skill | cursor-team-kit @ e46364b | markdown skill | PORT | C | Only cursor-team-kit skill pstack-claude imported (CHANGES 0.9.2); body has no Cursor/Claude constructs, copied verbatim, `menu-description` folded into `description` |
| thermo-nuclear-code-quality-review | skill | cursor-team-kit @ e46364b | markdown skill | ADAPT | C | Reference port pinned a layered Report Format (CHANGES 0.9.12) that overrides `swarm` aggregation; kept, with the swarm fan-out rewritten to OMP `task` batch (`task/types.ts:167-176`) and vendor slugs deleted |
| make-pr-easy-to-review | skill | cursor-team-kit @ e46364b | markdown skill | PORT | C | Audited clean in the reference port (`CHANGES.md:222`); `gh`/`git`/`bash` only, copied verbatim |
| fix-ci | skill | cursor-team-kit @ e46364b | markdown skill | PORT | C | `gh pr checks` flow, no Cursor constructs (`CHANGES.md:222`); verbatim |
| fix-merge-conflicts | skill | cursor-team-kit @ e46364b | markdown skill | PORT | C | `git`/lockfile workflow, no Cursor constructs (`CHANGES.md:222`); verbatim |
| get-pr-comments | skill | cursor-team-kit @ e46364b | markdown skill | PORT | C | `gh`-only (`CHANGES.md:222`); verbatim |
| what-did-i-get-done | skill | cursor-team-kit @ e46364b | markdown skill | PORT | C | `git log` summary flow (`CHANGES.md:222`); verbatim |
