...
After the explanation is complete, spawn one architectural critic per entry in your configured how-critics role list, all in a single `task` batch call (`{context, tasks[]}`).
...
Same framework as the **interrogate** skill (`skill://interrogate`). You're a pragmatic lead, not an aggregator.
