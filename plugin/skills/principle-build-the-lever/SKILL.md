...
**Balance:** The bar is triviality, not repetition. A one-off still earns a lever when the lever is what makes the work checkable. Per `skill://principle-laziness-protocol`, build the smallest script that does or proves the job, never a framework.

Distinct from `skill://principle-encode-lessons-in-structure`, which makes a recurring instruction a durable guardrail. This is throughput and reviewability on the work in front of you. For scripting the verification itself, see `skill://principle-prove-it-works`.
