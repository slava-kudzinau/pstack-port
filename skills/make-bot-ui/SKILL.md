...
upstream: pstack/skills/make-bot-ui/SKILL.md
upstream_sha: fd878692de15a3069c21c8f429eb0b9f2fe178fa
upstream_version: 0.14.5
status: adapted
note: 'make-bot-ui/SKILL.md documents Cursor''s own Routines/webhook-automation product surface: the `update_state` routine-creation tool, the `SendToUser` secret-request UI card, the Cursor chat-header/computer-preview navigation for copying the webhook URL and sender key, and the `[routine]`/`<webhook_event>` wake format (SKILL.md:12-50,104-114). None of these have a confirmed OMP equivalent — no automation/routine tool, no secret-request card, no webhook-triggered agent wake — and this port''s substitution contract has no mapping for them, so per its escape hatch they are preserved verbatim rather than guessed at. This port only fixes `name: Make Bot UI` to the directory-matching `name: make-bot-ui` (upstream had no `disable-model-invocation` field, so none is added). The page-hosting, secret-hygiene, and Tailscale sections (SKILL.md:52-102) are OS/network-level and port unchanged.'
---
