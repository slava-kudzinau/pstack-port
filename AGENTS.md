...
├── plugins/pstack/        # the Claude Code target — generated by tools/claude/apply.mjs
│   ├── .claude-plugin/plugin.json # name, version, skills/agents paths
│   ├── skills/<name>/SKILL.md     # generated, /pstack:<name> slash commands
│   ├── agents/*.md                # generated
│   ├── hooks/                     # hand-carried: SessionStart mandate + polyglot runner
│   └── models.json                # hand-carried: shipped default role-to-model policy
...
1. **Branding.** Shipped files (everything under `plugin/`) contain zero
   ...
   `scripts/branding-check.ts` enforces this; run it before any commit. This
   ban does not reach `plugins/pstack/` or `tools/claude/`: the Claude Code
   target is native Claude-dialect content by design (`claude`, `Skill` tool,
   `pstack:<name>` slash commands are correct there, not violations), and
   `branding-check.ts` still scans only the `plugin/` (OMP) tree.
...
- Touching `plugins/pstack/`, `tools/claude/`, or `docs/claude-code.md`:
  run `bun scripts/claude-check.ts` (tree invariants: no `commands/`, the
  hide keys, the denylist, both manifests) and
  `bun test tools/claude/apply.test.ts scripts/claude-check.test.ts`
  (exact files; the bare `scripts/` substring also matches unrelated vendored
  tests under `refs/`). `bun scripts/provenance.ts --check` already covers
  both Catalog tables in one run.
